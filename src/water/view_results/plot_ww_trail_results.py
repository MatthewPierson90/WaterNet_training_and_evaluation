    ...
    def __init__(self, batches=(1, 2, 3),
        ...
        for batch in batches:
            if self.raw is None:
                ...
                self.model = np.load(ppaths.training_data/f'model_results/training_batch_{batch}/y_model.npy')[:].astype(np.float32)
            else:
                raw = np.load(ppaths.training_data/f'model_results/training_batch_{batch}/raw.npy').astype(np.float32)
                model = np.load(ppaths.training_data/f'model_results/training_batch_{batch}/y_model.npy')[:].astype(np.float32)
