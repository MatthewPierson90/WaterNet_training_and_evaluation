...
if __name__ == '__main__':
    ...
    kwargs = dict(
        model_number=model_number, evaluation_dir=ppaths.model_inputs_832,
        num_per_load=max_per_it*2, max_per_it=max_per_it,
        data_loader=SenElBurnedLoader(), max_target=21, input_dir_name='test_data',
        output_dir_name='output_test_data', stats_name='evaluation_test',
    ...
    kwargs = dict(
        model_number=model_number, evaluation_dir=ppaths.model_inputs_832,
        num_per_load=max_per_it*2, max_per_it=max_per_it,
        data_loader=SenElBurnedLoader(), max_target=21, input_dir_name='val_data',
        output_dir_name='output_val_data', stats_name='evaluation_val',
        loss_func_type=WaterwayLossDecForEval, num_y=1
    )
    p = Process(target=evaluate_on_all_sen_data_multi, kwargs=kwargs)
    p.start()
    ...
    kwargs = dict(
        ...
        data_loader=SenElBurnedLoader(), max_target=21, input_dir_name='input_data',
        output_dir_name='output_data', stats_name='evaluation_stats',
        loss_func_type=WaterwayLossDecForEval, num_y=1
    )
    p = Process(target=evaluate_on_all_sen_data_multi, kwargs=kwargs)
    p.start()
